...
(setq zemp-player-process '(start-process zemp-player zemp-player-buffer zemp-player))
(setq zemp-player-process-args '("--no-config" "--no-loop-file"  "--no-video"  "--no-audio-display" "--force-window=no"))
...
(defun zemp-play(track)
  ...
  (message track)
  (eval (append zemp-player-process zemp-player-process-args (list (format "--input-ipc-server=%s" zemp-player-mpvsocket) track)))
  )
